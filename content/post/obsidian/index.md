...
Using Shell-commands and buttons a note can be transformed into a control centre from which the local server can be launched, the generated site accessed and the server stopped again. 
![My control centre note.](control.png)
While Hugo Blox prominently advertises this feature, to my knowledge there is no reason that the same could not be done for a site generated with Jekyll for example or any other static site generator as long as it supports deployment to GitHub Pages via an action.
...
The `.md` files for posts can either be placed in the `post` folder directly or inside a folder with the same name, with the source file named `index.md`. The latter enables the placement of images for the post directly inside the folder as seen in the example below.
