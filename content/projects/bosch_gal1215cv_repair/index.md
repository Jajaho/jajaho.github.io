...
The charger's specifications according to the manual:  
...
The following is my own theory and is only supported by a couple of measurements.
The charger circuit is visibly divided into two sections: The primary, high voltage side of the SMPS and the secondary controller side. The secondary side has two supplies VDD and VCC. VCC powers the microcontroller (uC, a custom jobby by ST-Microelectronics) and other ICs. VDD is the battery voltage plus one forward voltage drop. When no battery is connected the VDD idles at 1.6 V and the VCC at 13 V. 

When a battery is detected by the microcontroller the supply voltage is set to induce the desired charging current or constant voltage. 
The supply voltage is increased via the feedback path over the optocoupler U1. By increasing the photo-transistor's c/e-resistance the Vbe of V1 is decreased which again increases Vgs of V5, letting current flow for a longer portion of a period in the switching cycle.
...
The kicker however is that inside the Bosch GSR BAT411 both C1 and C2 are not actually connected to the batteries. I have confirmed myself that this is also the case for the Dremel battery.
...
a) One cell gets over-discharged, or  
